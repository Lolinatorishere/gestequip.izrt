...
function on_request_rem_load($data_request , $pdo){
    $ret = array();
...
function on_request_rem_refresh($data_request , $pdo){
...
function read_request_rem($data_request , $pdo){
    ...
    if(!isset($data_request["refresh"])){
        return on_request_rem_load($data_request , $pdo);
    }else{
        return on_request_rem_refresh($data_request , $pdo);
    }
}
